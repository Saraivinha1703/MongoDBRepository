    ...
    where T : IEntity<TKey>
{
    ...
    public virtual async Task<IEnumerable<T>> GetAsync(Expression<Func<T, bool>>? filter = null, FindOptions? defaultOptions = null, CancellationToken? cancellationToken = null)
    {
        ...
        {
            List<T> result;
            IUserInfo<TKey>? userInfo = _userInfoProvider.GetUserInformation<TKey>();
    ...
    public virtual async Task<T?> GetByIdAsync(TKey id, FindOptions? defaultOptions = null, CancellationToken? cancellationToken = null)
    {
        try 
        {
            T result;
    ...
    public virtual Task<IQueryable<T>> GetQueryableAsync(FindOptions? defaultOptions = null, CancellationToken? cancellationToken = null)
    {
        try 
        {
            ...
            if(userInfo != null && userInfo.Roles.Contains(_context.DefaultDatabaseName))
            {
